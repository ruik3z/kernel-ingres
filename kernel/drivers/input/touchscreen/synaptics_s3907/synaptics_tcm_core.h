...
/* SPI Pinctrl setup */
#define PINCTRL_STATE_SPI "pmx_syna_spi_mode"
...
#define SHOW_PROTOTYPE(m_name, a_name)                                         \
	static ssize_t CONCAT(m_name##_sysfs, _##a_name##_show)(               \
...
#define STORE_PROTOTYPE(m_name, a_name)                                        \
	static ssize_t CONCAT(m_name##_sysfs, _##a_name##_store)(              \
		struct device * dev, struct device_attribute * attr,           \
		const char *buf, size_t count);                                \
	static struct device_attribute dev_attr_##a_name =                     \
		__ATTR(a_name, (S_IWUSR | S_IWGRP), syna_tcm_show_error,       \
...
#define KOBJ_STORE_PROTOTYPE(m_name, a_name)                                   \
	static ssize_t CONCAT(m_name##_sysfs, _##a_name##_store)(              \
		...
		const char *buf, size_t count);                                \
	static struct kobj_attribute kobj_attr_##a_name =                      \
		__ATTR(a_name, (S_IWUSR | S_IWGRP), NULL,                      \
		       CONCAT(m_name##_sysfs, _##a_name##_store));
...
enum fod_status {
	FOD_STATUS_INVALID =
		-1, /*as default value, means fingerprint is not enabled*/
	FOD_STATUS_DISABLED = 0, /*means fingerpirint is not enabled*/
	FOD_STATUS_AUTHENTICATION =
		1, /* means fingerprint is in authentication*/
	FOD_STATUS_INPUT_FINGERPRINT = 2, /*means fingerprint is in enroll*/
	FOD_STATUS_IN_SUSPENDING =
		3, /*means fingerprint is to be set when suspend*/
	FOD_STATUS_DELETED = 100, /*as all fingers in the system is deleted*/
};
...
struct syna_tcm_hcd {
	...
	int irq;
	bool do_polling;
	bool flag_sleep; /*Do not operate ic sleep when resume*/
	bool in_suspend;
	bool in_suspending;
	bool irq_enabled;
	bool in_hdl_mode;
	...
	int (*reset_n_reinit)(struct syna_tcm_hcd *tcm_hcd, bool hw,
			      bool update_wd);
	...
	int (*read_message)(struct syna_tcm_hcd *tcm_hcd, unsigned char *in_buf,
			    unsigned int length);
