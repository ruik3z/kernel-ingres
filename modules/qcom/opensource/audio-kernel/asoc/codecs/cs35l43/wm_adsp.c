				   ...
				   const void *buf, size_t len)
{
	...
	adsp_dbg(dsp, "Wrote %zu bytes <buf[0]= 0x%08x> to addr:%x( ctl name is %s)\n", 
		len, *((unsigned int*)scratch), reg, ctl->name);
			       ...
			       const void *buf, size_t len)
{
	...
	adsp_dbg(dsp, "write %s: ctl->enabled:%d, ctl->dsp->running:%d, buf = 0x%08x, ctl->cache=0x%08x\n", 
		ctl->name, ctl->enabled, ctl->dsp->running, *((int*)buf), *((int*)ctl->cache));
...
static int wmfw_add_ctl(struct wm_adsp *dsp, struct wm_coeff_ctl *ctl)
{
	...
	adsp_dbg(dsp, "%s: success add component controls : %s, ret:%d\n",
		__func__, kcontrol->name, ret);
...
static int wm_adsp_load(struct wm_adsp *dsp)
{
	...
	file[PAGE_SIZE - 1] = '\0';
	adsp_info(dsp, "dsp start to load algo file: %s\n",file);
...
int wm_adsp_load_coeff(struct wm_adsp *dsp)
{
	...
	file[PAGE_SIZE - 1] = '\0';
	adsp_info(dsp, "dsp start to load algo coeff file: %s\n",file);
	ret = request_firmware(&firmware, file, dsp->dev);
	if (ret != 0) {
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	struct wm_adsp *dsp = &dsps[mc->shift - 1];
	char preload[32];
	adsp_dbg(dsp, "wm_adsp2_preloader_put: %d\n", ucontrol->value.integer.value[0]);
	...
	adsp_dbg(dsp, "snd_soc_dapm_sync preload enable finished\n");
	...
	adsp_dbg(dsp, "snd_soc_dapm_sync preload disable finished\n");
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	struct wm_adsp *dsp = &dsps[w->shift];
	struct wm_coeff_ctl *ctl;
	adsp_dbg(dsp, "wm_adsp_early_event(%d) start\n", event);
	...
	adsp_dbg(dsp, "wm_adsp_early_event(%d) end\n", event);
...
static int wm_halo_apply_default(struct snd_soc_dapm_widget *w)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
	struct wm_adsp *dsps = snd_soc_component_get_drvdata(component);
	struct wm_adsp *dsp = &dsps[w->shift];
		  ...
		  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		mutex_unlock(&dsp->pwr_lock);
		adsp_info(dsp, "%s, dsp->running: %d dsp->booted: %d, execution started\n",
				__func__, dsp->running, dsp->booted);
		break;
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		adsp_info(dsp, "%s, dsp->running: %d dsp->booted: %d, execution stopped\n",
				__func__, dsp->running, dsp->booted);
		break;
